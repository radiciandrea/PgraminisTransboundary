...
load("Data/B_C_i_cnc_df_val1718.RData") 
...
Mc <- as.matrix(B_C_nc_i_v)
diag(Mc) = 1

Mnc <- as.matrix(B_C_c_i_v)
diag(Mnc) = 1
...
for(i in 1:nrow(ncimnc_df)) { 
  ...
  for(k in 1:n_countries) {
    ...
    if(i == 1) {
      covered_cells <- which(Mc[,ncimnc_df$sentinel[1]]>0)
    } else {
      covered_cells <- which(rowSums(Mc[,ncimnc_df$sentinel[1:i]])>0)
    }
...
for(j in 1:nrow(cisc_df)) {
  for(k in 1:n_countries) {
    ...
    if(j == 1) {
      covered_cells <- which(Mnc[,cisc_df$sentinel[1]]>0)
    } else {
      covered_cells <- which(rowSums(Mnc[,cisc_df$sentinel[1:j]])>0)
    }
...
save(national_coverage_list, file = "Data/national_coverage_list_val1718.RData")
